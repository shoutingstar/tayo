...
import kr.co.tayo.dao.A_replylistDAO;
...
@Service
public class A_replylistService {
	
	Logger logger =LoggerFactory.getLogger(getClass());
	
	@Autowired A_replylistDAO dao;

	public HashMap<String, Object> A_replylist(int page) {
		...
		ArrayList<A_replylistDTO> list=dao.A_replylist(offset);
	...
	public ArrayList<A_replylistDTO> A_replylistsearch(HashMap<String, String> param) {
		logger.info("DB필요해서 DAO 호출");
		logger.info("param:"+ param);
		A_replylistDTO dto= new A_replylistDTO();	          
		return dao.A_replylistsearch(param); 
	}
	
	public int A_replylistdel(ArrayList<String> delList) {
		int total=0;		
		for (String id : delList) {
			total += dao.A_replylistdel(id);
		}
