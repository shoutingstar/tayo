		...
		<table id="detail">
			...
			<tr>
				<td colspan="2" class="btn_area">
					...
					<button type="button" onclick="location.href='./TogetherupdateForm?num=${total.num}'">수정하기</button>				
					</c:if>
					<button type="button" onclick="location.href='./Togetherlist'">목록</button>
					<c:if test="${(sessionScope.loginId eq total.mem_id) || sessionScope.power>0}">
					<button type="button" onclick="location.href='./Togetherdelete?num=${total.num}'">삭제</button>
					</c:if>
				</td>
			</tr>		
		</table>
...
var memPower='${sessionScope.power}'; // 비공개 처리를 위한 관리자 권한 [수정]
...
function drawList2(list, reReListBtn, reNum){
	...
	if(list.length > 0){
		for(var i=0; i<list.length; i++){
			...
			if(loginId==list[i].mem_id){
				recontent += 		'<button onclick="reReplyModify(this)">'+'수정'+'</button> &emsp;';
			}
			if(memPower>0 || loginId==list[i].mem_id){
	...
	if(loginId.length == ""){
		...
		recontent += 		"<div>";
		recontent += 		'<input type="text" style="width:350px;" placeholder="답글을 입력해 주세요" value=""/></input>';
