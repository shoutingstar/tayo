...
.closeBtn { /*팝업 x 버튼*/
	...
	right: 15px;
	background-image:
		url("https://korean.visitkorea.or.kr/resources/images/theme/travel100scene/btn_pop_close03.png");
	font-weight: 900;
	width: 35px;
	height: 35px;
	border-radius: 50%;
...
#list-more-btn {
	...
	width: 46px;
	height: 46px;
	background: url(/public/static/img/icon/ic_list_plus.svg) center
		no-repeat;
	border-radius: 50%;
}
...
button .good {
	...
	font-size: 0;
	background-image:
		url("https://korean.visitkorea.or.kr/resources/images/theme/travel100scene/btn_m_good.png");
...
.btn button.good {
	...
	height: 20px;
	background:
		url("https://korean.visitkorea.or.kr/resources/images/theme/travel100scene/btn_good01.png")
		no-repeat;
}
...
<script>
var ha_value = '${param.ha_value}';

hashCarList(ha_value);
function hashCarList(ha_value){
	// 조회수 기준으로 가져오기
	$.ajax({
		type:	'GET',
		url:	'hashCarListCall',
...
function drawlist(data){
	var cont = "";
	var hash = data.hashCarList;
	var file = data.hashFile;
	console.log(hash.ca_name);
	console.log(hash);
	console.log(file);
	
	for(var i=0; i<hash.length; i++){
		
		if(file[i].newFileName == "1668132777766.png" || file[i].newFileName == "1668483894313.png" || file[i].newFileName == "1668483948802.png" || file[i].newFileName == "1668484306753.png"){	// 모닝은 크기가 달라서..
			cont += '<div id="a" onclick="CarSpl('+hash[i].ca_num+')" value="'+hash[i].ca_num+'"  style="z-index:0;">'; // 간편보기
			cont += '<span class="btn" id="listJjim"></span>';	// 찜 버튼
			cont += '<span class="category">'+hash[i].ca_model+'</span>'; // 차량 모델
			cont += '<div class="img" style="z-index:1;"><img id="readPhoto" src="/photo/'+file[i].newFileName+'" style="width:80%;"/></div>';
			cont += '<div class="item_box">';
			cont += '<h3 class="name">'+hash[i].ca_name+'</h3>';
			cont += '<div class="option_box">';
			cont += '<i class="point"></i>';
			cont += '<ul class="option_list">';
			cont += '<li>'+hash[i].ca_brand+'&nbsp;｜&nbsp;</li>';
			cont += '<li>'+hash[i].ca_age+'&nbsp;｜&nbsp;</li><br/>';
			cont += '</ul>';
			cont += '<p class="pice">'+hash[i].ca_price+'&nbsp원</p>';
			cont += '</div>';
			...
			cont += '<span class="btn" id="listJjim"></span>';	// 찜 버튼
			cont += '<span class="category">'+hash[i].ca_model+'</span>'; // 차량 모델
			cont += '<div class="img" style="z-index:1;"><img id="readPhoto" src="/photo/'+file[i].newFileName+'" style="width:100%;"/></div>';
			cont += '<div class="item_box">';
			cont += '<h3 class="name">'+hash[i].ca_name+'</h3>';
			cont += '<div class="option_box">';
			cont += '<i class="point"></i>';
